...
// Do XML HTTP request
	_doRequest: function(anEvent, aQuery) {
		...
		} else if (anEvent == 'refresh') {//alert("PL.state="+PL.state);
		...
		/*if(PL.parameters!=null && PL.parameters.length > 0) {
...
// On response  optional callback fun is called with optional user data.
	_getXML: function(url, callback) {//alert("callback="+callback);
		...
		if (cb) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4) {
					if (xmlhttp.status == 200) {//alert("xmlhttp.responseXML="+xmlhttp.responseXML);
...
/** Handle incoming events from server. */
	_onEvent: function (event) {
		...
		if (eventType == 'data') {
			...
			var timeout = event.get('p_wait');//alert("timeout="+timeout);
			setTimeout(function () {
...
/**  Handle XMLHttpRequest response XML. */
	_onResponse: function(xml) {//alert("xml="+xml);
